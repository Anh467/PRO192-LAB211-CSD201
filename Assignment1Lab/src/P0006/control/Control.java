...
//        model.getNum()[i]= model.getNum()[j]- model.getNum()[i];
    ...
    public static void binary(int []arr, int i, int j, int numFind){
        if(numFind==arr[(i+j)/2])
            model.setIndex((i+j)/2);
        else if(i<j){
            if(numFind> arr[(i+j)/2]) binary(arr, ((i+j)/2) +1, j, numFind);
            else if(numFind< arr[(i+j)/2]) binary(arr, i, ((i+j)/2) -1, numFind);
        } else model.setIndex(-1);
    }
    public static void binarySearch(int numFind){
        binary(model.getNum(),0, model.getNum().length-1, numFind);
        output(numFind);
    }
    public static void output(int numFind){
        System.out.print("Sorted array  : [");
        Control.view();
        if(model.getIndex()!=-1)
            System.out.println("Found "+numFind+" at index: "+model.getIndex());
